...
const Catcher = {
  ...
  _updateInterceptBtn() {
    ...
    if (this._interceptOn) {
      ...
      if (addr) { addr.textContent = 'Proxy: localhost:' + (this._proxyPort || 8080); addr.classList.remove('hidden'); }
    } else {
  ...
  _startPolling() {
    this._stopPolling();
    this._pollPending();
    this._pollTimer = setInterval(() => this._pollPending(), 1500);
  },
  ...
  async _pollPending() {
    try {
      ...
      if (pendingRes.ok) { this._pending = await pendingRes.json(); this._renderPending(); }
        ...
        return `<div class="catcher-pending-item border-b border-pink-500/10 bg-pink-500/[0.02]">
          ...
          <div class="px-3 pb-3 space-y-2">
            ...
            <div>
              ...
              <textarea id="${rid}-headers" class="w-full h-24 p-2 rounded bg-base-900/60 border border-white/5 text-[10px] font-mono text-gray-300 resize-none focus:outline-none focus:border-pink-500/40" onchange="Catcher._editField('${r.id}','headers',this.value)">${this._esc(headersStr)}</textarea>
            </div>
            <div>
              <div class="text-[9px] uppercase text-gray-500 font-semibold mb-1">Body</div>
              <textarea id="${rid}-body" class="w-full h-32 p-2 rounded bg-base-900/60 border border-white/5 text-[10px] font-mono text-gray-300 resize-none focus:outline-none focus:border-pink-500/40" onchange="Catcher._editField('${r.id}','body',this.value)">${this._esc(bodyStr)}</textarea>
            </div>
          </div>
        </div>`;
      }
