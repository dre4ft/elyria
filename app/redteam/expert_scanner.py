"""
Expert Pentest Scanner — Documentation-aware, master-prompt-driven,
high-round deep scan with analysis→exploration feedback loop.

Built alongside ai_scanner.py — does not modify existing AIScanner.
Reuses its session, providers, tools, and request execution via inheritance.
"""
...
class ExpertAIScanner(AIScanner):
    ...
    - Master prompt + documentation + OpenAPI context
    - 2x default rounds (explore=30, analysis=15)
    - Analysis→exploration feedback loop
    - Smarter context window management (summarization)
    - Latest scan logs as analysis tool input
    """
    ...
    # ── Override: documentation-aware context block ──

    def _build_context_block(self):
        parts = [super()._build_context_block()]

        if self.master_prompt:
            parts.insert(0, f"## MASTER PROMPT (highest priority instructions)\n{self.master_prompt}\n")

        if self.openapi_spec:
            spec_preview = self.openapi_spec[:8000] if len(self.openapi_spec) > 8000 else self.openapi_spec
            parts.append(f"## OpenAPI / API Documentation\n```\n{spec_preview}\n```\n")
            if len(self.openapi_spec) > 8000:
                parts.append("(spec truncated to 8000 chars — full spec available via tool)\n")

        if self.documentation:
            doc_preview = self.documentation[:6000] if len(self.documentation) > 6000 else self.documentation
            parts.append(f"## Custom documentation\n{doc_preview}\n")
            if len(self.documentation) > 6000:
                parts.append("(doc truncated to 6000 chars)\n")

        return "\n".join(parts)

    # ── Context summarizer (keep window manageable) ──

    def _summarize_history(self, msgs, keep_last=20):
        """Use the pro model to summarize older conversation turns."""
        if len(msgs) <= keep_last + 10:
            return msgs  # not enough to worry

        # Build summary prompt
        recent = msgs[-keep_last:]
        older = msgs[1:-keep_last]  # skip system msg

        summary_prompt = {
            "role": "user",
            "content": (
                "Summarize the key findings, tested endpoints, anomalies discovered, and "
                "vulnerabilities confirmed from the conversation above. Be concise but complete. "
                "Include: endpoints probed, response patterns, auth behavior, and any findings. "
                "Format as bullet points. Keep it under 2000 characters."
            ),
        }
        older.append(summary_prompt)
        try:
            resp = self.flash.chat(older, tools=None)
            summary = resp.get("content", "")[:2000]
        except Exception:
            return msgs  # summarization failed, keep going

        new_system_content = (
            msgs[0]["content"] +
            "\n\n## Session summary (previous rounds compressed)\n" + summary
        )
        return [{"role": "system", "content": new_system_content}] + recent

    # ── Enhanced exploration prompts (30 rounds, documentation-aware) ──
...
For ANY question answered NO: call pentest_make_requests NOW. Then verify. Then call pentest_add_findings. Leave nothing untested.""",
    ...
    def _get_expert_tools(self):
        tools = _get_tools()
        tools.append({
            "type": "function",
            "function": {
                "name": "pentest_read_logs",
                "description": "Read recent scan logs (request/response history). Use this in analysis to review what happened in exploration. Returns up to 30 recent log entries.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "limit": {"type": "integer", "description": "Number of log entries to read (max 30)", "default": 20},
                    },
                },
            },
        })
        ...
        try:
            ...
            for l in logs:
                entries.append({
                    ...
                    "status": l.get("response_status", 0),
                    "check_name": l.get("check_name", ""),
                    "time_ms": l.get("response_time_ms", 0),
                    "body_preview": (l.get("response_body_preview") or "")[:200],
                    "request_url": (l.get("request_url") or "")[:120],
                })
            return json.dumps(entries, default=str)
        except Exception as e:
            return json.dumps({"error": str(e)})

    # ── Tool index override (add read_logs) ──

    async def _execute_tool(self, name, args):
        if name == "pentest_read_logs":
            return await self._handle_read_logs(args)
        return await super()._execute_tool(name, args)

    # ── Main expert scan ──

    async def run(self):
        """Expert deep scan — documentation-driven, analysis↔exploration feedback loop."""
        ai_findings = []
        ...
        # ── Shared tool-call processor (same logic as AIScanner) ──

        async def _process_tool_calls(msgs, resp):
            _add_tokens(resp)
            tc_raw = resp.get("tool_calls") or []
            if not tc_raw:
                tc_raw = _extract_tc_text(
        ...
        # ══════════════════════════════════════════════════════════
        # EXPERT SCAN — interleaved explore↔analyze with feedback
        # ══════════════════════════════════════════════════════════

        explore_prompts = self._get_exploration_prompts()
        analysis_prompts = self._get_analysis_prompts()

        master_instructions = ""
        if self.master_prompt:
            master_instructions = f"\nMASTER INSTRUCTIONS (follow precisely):\n{self.master_prompt}\n"

        doc_instructions = ""
        if self.openapi_spec or self.documentation:
            doc_instructions = (
                "\nYou have access to API documentation and an OpenAPI specification. "
                "Use it to guide your testing — target documented endpoints, validate "
                "documented schemas, and identify gaps between documentation and reality.\n"
            )

        system = {"role": "system", "content": f"""You are an EXPERT penetration testing agent operating in documentation-driven mode.
...
- The documentation is your guide — test what's documented AND what's missing
- Cross-reference actual API behavior against the spec at every opportunity
- Never describe what you would do — actually DO it by calling tools"""}
        ...
        total_analyze = min(len(analysis_prompts), self.analysis_rounds)

        # Interleave: 3 explore rounds per analysis round
        explore_per_analyze = 3
        explore_idx = 0
        analyze_idx = 0
        summarize_every = 8  # compress context every 8 rounds

        while explore_idx < total_explore or analyze_idx < total_analyze:
            # ── Exploration batch (flash model, fast) ──
            batch_end = min(explore_idx + explore_per_analyze, total_explore)
            for i in range(explore_idx, batch_end):
                msgs.append({"role": "user", "content": explore_prompts[i]})
                try:
                    _beat()
                ...
                if await _process_tool_calls(msgs, resp):
                    ...
                    msgs.append({"role": "user", "content": "You MUST call pentest_make_requests. Make at least 5 requests now."})
                    try:
                        _beat()
                        resp = self.flash.chat(msgs, tools=tools)
                        _add_tokens(resp)
                ...
                if self.callbacks.get("on_progress"):
                    pct = int(40 + (i + 1) * 30 / total_explore)
                    self.callbacks["on_progress"](pct, f"Expert explore {i + 1}/{total_explore}")

                # Periodic context compression
                if (i + 1) % summarize_every == 0 and len(msgs) > 30:
                    msgs = self._summarize_history(msgs)
                    msgs.append({"role": "user", "content": "(Context compressed. Continue from the summary above. Call pentest_make_requests for your next batch.)"})

            explore_idx = batch_end

            # ── Analysis round (pro model, deep reasoning) ──
            if analyze_idx < total_analyze:
                if analyze_idx == 0:
                    msgs[0] = {"role": "system", "content": f"""You are an EXPERT penetration tester performing deep analysis.
...
Be thorough. Documentation gaps are findings too."""}
                ...
                try:
                    ...
                    if not await _process_tool_calls(msgs, resp):
                        msgs.append({"role": "user", "content": "Call pentest_read_logs first, then pentest_add_findings for every vulnerability, and pentest_make_requests for verification."})
                        try:
                            _beat()
                            resp = self.pro.chat(msgs, tools=tools)
                            _add_tokens(resp)
        ...
        # ══════════════════════════════════════════════════════════
        # PHASE 3 — Expert Report Writing (pro model, 5 rounds)
        # ══════════════════════════════════════════════════════════
        report = ""
        report_rounds = 5
        findings_summary = "\n".join(
        ...
        report_system = {"role": "system", "content": f"""You are an EXPERT penetration testing report writer at a top-tier cybersecurity firm (ESN).

You have just completed a deep penetration test of: {self.target}

{context if context else ''}

FINDINGS SUMMARY ({len(ai_findings)} total):
{findings_summary[:3000]}

Severity breakdown: {sev_summary}

SCAN METRICS:
- Exploration rounds: {total_explore}
- Analysis rounds: {analyze_idx}
- Flash model: {self.flash_model}
- Pro model: {self.pro_model}
- Tokens used: {ai_tokens.get('total', 0):,}
- Master prompt used: {bool(self.master_prompt)}

TASK: Write a professional penetration test audit report suitable for delivery to a client CISO.
Write in French. Use Markdown formatting. Be precise, factual, and actionable.

The report MUST follow this exact structure:

# Rapport d'Audit de Securite — Pentest Expert Elyria

## 1. Synthese Manageriale
- Resume executif (1 paragraphe)
- Niveau de risque global (Critique / Eleve / Modere / Faible)
- Recommandations prioritaires (top 3)

## 2. Demarche et Perimetre
- Cible auditée
- Perimetre fonctionnel teste
- Methodologie employée (OWASP API Top 10, tests boite grise)
- Moyens IA deployes (modeles, rounds, tokens)

## 3. Schemas d'Attaque
Pour chaque vecteur d'attaque majeur identifie, decrire :
- Scenario d'attaque (etapes)
- Preconditions
- Impact metier

## 4. Synthese des Vulnerabilites
Tableau recapitulatif : | Severite | Titre | Endpoint | CVSS | CWE | Impact |
Puis, pour chaque finding CRITICAL et HIGH :
- Description detaillee
- Preuve d'exploitation
- Risque selon ISO 27005 (probabilité x impact = risque)
- Recommandation de remediation

## 5. Analyse des Risques (ISO 27005)
- Matrice des risques identifiés
- Classification par type d'actif impacte (confidentialite, integrite, disponibilite)
- Niveau de risque residuel apres remediation

## 6. Synthese IA
- Repartition des rounds par theme d'attaque
- Modeles utilises et leur role (flash = exploration, pro = analyse + rapport)
- Tokens consommes
- Couverture de l'OWASP API Top 10
- Recommandations pour les prochains audits

## 7. Annexes
- Liste complete des findings
- Tableau de bord des indicateurs cles
...
IMPORTANT: Write the COMPLETE report. Do not abbreviate. Each section must be fully fleshed out."""}
        ...
        if report:
            try:
                ...
                conn.execute(
                    ...
                    (report, report, self.campaign_id),
                )
                conn.commit()
                conn.close()
            except Exception:
        ...
        return {
            "findings": ai_findings, "tokens": ai_tokens,
            "flash_model": self.flash_model, "pro_model": self.pro_model,
            "explore_rounds": total_explore, "analysis_rounds": analyze_idx,
            "mode": "expert", "master_prompt_used": bool(self.master_prompt),
